...
public class InventarioController {
	...
	@RequestMapping(value = "/ver/{estado}/", method = RequestMethod.GET)
    public String obtenerSolicitudesCompletas(Model model,@PathVariable("estado") String estado) throws ParseException { 	
    	logger.debug("Mostrando Pagina de busqueda de solicitudes completas en JSP");
    	UserSistema usuarioActual = this.usuarioService.getUser(SecurityContextHolder.getContext().getAuthentication().getName());
    	List<Solicitud> solicitudesCompletas = this.solicitudesService.getSolicitudesporEstado(usuarioActual.getUsername(),estado);
    	model.addAttribute("solicitudes", solicitudesCompletas);
    	for (Solicitud solicitud:solicitudesCompletas) {
    		MessageResource mr = null;
    		String descCatalogo = null;
    		mr = this.messageResourceService.getMensaje(solicitud.getEstSolicitud(),"CAT_EST_SOL");
    		descCatalogo = (LocaleContextHolder.getLocale().getLanguage().equals("en")) ? mr.getEnglish(): mr.getSpanish();
