  ...
  <div class="app-body">
    ...
    <main class="main">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"><spring:message code="centers" /></li>
        <!-- Breadcrumb Menu-->
      ...
      <div class="animated fadeIn">
          <div class="card">
            <div class="card-header">
              <i class="fa fa-building"></i> <spring:message code="centers" />
              <div class="card-actions">
              </div>
            </div>
            <div class="card-body">
              <spring:url value="/admin/centers/newCenter/"	var="newCenter"/>	
              <button id="lista_centros_new" onclick="location.href='${fn:escapeXml(newCenter)}'" type="button" class="btn btn-outline-primary"><i class="fa fa-plus"></i>&nbsp; <spring:message code="add" /></button><br><br>	
              <table id="lista_centros" class="table table-striped table-bordered datatable" width="100%">
                <thead>
                	<tr>
	                    <th><spring:message code="idCentro" /></th>
	                    <th class="hidden-xs"><spring:message code="nombreCentro" /></th>
	                    <th class="hidden-xs"><spring:message code="puedeComprar" /></th>
                ...
                <tbody>
                	<c:forEach items="${centros}" var="centro">
                		<tr>
                			<spring:url value="/admin/centers/{centro}/"
                                        var="centroUrl">
                                <spring:param name="centro" value="${centro.idCentro}" />
                            </spring:url>
                            <spring:url value="/admin/centers/editCenter/{centro}/"
                                        var="editUrl">
                                <spring:param name="centro" value="${centro.idCentro}" />
                            </spring:url>
                            <spring:url value="/admin/centers/habdes/disable1/{centro}/"
                                        var="disableUrl">
                                <spring:param name="centro" value="${centro.idCentro}" />
                            </spring:url>
                            <spring:url value="/admin/centers/habdes/enable1/{centro}/"
                                        var="enableUrl">
                                <spring:param name="centro" value="${centro.idCentro}" />
                            </spring:url>
                            <td><a href="${fn:escapeXml(centroUrl)}"><c:out value="${centro.idCentro}" /></a></td>
                            <td class="hidden-xs"><c:out value="${centro.nombreCentro}" /></td>
                            <c:choose>
                            ...
                            <td>
                                ...
                                <c:choose>
								<c:when test="${centro.pasive=='0'.charAt(0)}">
									<a href="#" data-toggle="modal" data-nomitem="${centro.nombreCentro}" data-whatever="${fn:escapeXml(disableUrl)}" class="btn btn-outline-primary btn-sm desact"><i class="fa fa-trash-o"></i></a>
								</c:when>
								<c:otherwise>
									<a href="#" data-toggle="modal" data-nomitem="${centro.nombreCentro}" data-whatever="${fn:escapeXml(enableUrl)}" class="btn btn-outline-primary btn-sm act"><i class="fa fa-check"></i></a>
								</c:otherwise>
							</c:choose>
                            </td>
                		</tr>
