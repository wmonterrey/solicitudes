...
<body class="app header-fixed sidebar-fixed aside-menu-fixed aside-menu-hidden">
  ...
  <div class="app-body">
    ...
    <main class="main">
      	...
      	<div class="animated fadeIn">
            ...
            <div class="row">
	            <div class="col-md-12">
	              <div class="card">
	                <div class="card-header bg-dark text-white">
	                  <i class="icon-check"></i>&nbsp;<strong><spring:message code="centers" /></strong>
	                </div>
	                <div class="card-body">
	                	<table id="lista_centros" class="table table-striped table-bordered datatable" width="100%">
			                <thead>
			                	<tr>
				                    <th><spring:message code="centers" /></th>
									<th><spring:message code="enabled" /></th>
									<th><spring:message code="addedBy" /></th>
									<th><spring:message code="dateAdded" /></th>
									<th><spring:message code="actions" /></th>
			                ...
			                <tbody>
			                	<c:forEach items="${usuarioscentros}" var="centro">
								<tr>
									<spring:url value="/admin/users/disableCentro/{username}/{center}/" var="disableCentroUrl">
		                               <spring:param name="username" value="${centro.userCenterId.username}" />
		                               <spring:param name="center" value="${centro.userCenterId.center}" />
		                            </spring:url>
		                            <spring:url value="/admin/users/enableCentro/{username}/{center}/" var="enableCentroUrl">
		                               <spring:param name="username" value="${centro.userCenterId.username}" />
		                               <spring:param name="center" value="${centro.userCenterId.center}" />
		                            </spring:url>
									<td><c:out value="${centro.centro.nombreCentro}" /></td>
									<c:choose>
									...
									<c:choose>
										<c:when test="${centro.pasive=='0'.charAt(0)}">
											<td><a data-toggle="modal" data-nomitem="${centro.centro.nombreCentro}" data-whatever="${fn:escapeXml(disableCentroUrl)}" class="btn btn-outline-primary btn-sm desact"><i class="fa fa-trash-o"></i></a></td>
										</c:when>
										<c:otherwise>
											<td><a data-toggle="modal" data-nomitem="${centro.centro.nombreCentro}" data-whatever="${fn:escapeXml(enableCentroUrl)}" class="btn btn-outline-primary btn-sm act"><i class="fa fa-check"></i></a></td>
										</c:otherwise>
									</c:choose>
								</tr>
							</c:forEach>
	                ...
	                <div class="card-header">
	                    <div class="row float-right mr-4" >
			            	<spring:url value="/admin/users/addCentro/{username}/" var="addCentroUrl"><spring:param name="username" value="${user.username}" /></spring:url>
	  ...
	  <!-- /.modal-dialog -->
	  <!-- Modal -->
  	  <div class="modal fade" id="centrosForm" data-role="centrosForm" data-backdrop="static" data-aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h2 class="modal-title"><spring:message code="add" /> <spring:message code="centers" /></h2>
				</div>
				<div class="modal-body">
					...
					<div id="cuerpo">
						<fieldset class="form-group">
		                 	<i class="fa fa-check"></i>
		                    <label><spring:message code="centers" /></label>
